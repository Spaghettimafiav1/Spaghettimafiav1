--[[
    Spaghetti Mafia Hub v1 (FORCE START VERSION)
    
    Fixes:
    - FORCED UI VISIBILITY: Prevents the UI from getting stuck at Size 0.
    - SAFE WHITELIST: Bypasses network hangs instantly.
    - EXECUTOR COMPATIBILITY: Works on Solara, Incognito, Electron, etc.
]]
...
-- Safe Service Call for VirtualUser
...
--// 2. CLEANUP & VARIABLES
-- SAFE GUI PARENTING (CRITICAL FIX)
...
LoadBox.BackgroundTransparency = 0 -- FORCE VISIBLE
...
PastaIcon.TextTransparency = 0 -- FORCE VISIBLE
...
Library:AddGlow(MiniPasta, Settings.Theme.Gold, 1.5); 
local CheckDrag = Library:MakeDraggable(MiniPasta, true) 

local MainFrame = Instance.new("Frame", ScreenGui); 
local NEW_WIDTH = 550
local NEW_HEIGHT = 420 
-- FORCE SIZE IMMEDIATELY TO PREVENT INVISIBLE GUI
...
-- Animation Logic (Now optional, won't hide the frame)
...
local CloseBtn = Instance.new("TextButton", TopBar); CloseBtn.Size = UDim2.new(0, 30, 0, 30); CloseBtn.Position = UDim2.new(1, -45, 0, 15); CloseBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30); CloseBtn.Text = "_"; CloseBtn.TextColor3 = Settings.Theme.Gold; CloseBtn.Font=Enum.Font.GothamBold; CloseBtn.TextSize=18; Library:Corner(CloseBtn, 8); Library:AddGlow(CloseBtn, Settings.Theme.Gold, 1.5)
