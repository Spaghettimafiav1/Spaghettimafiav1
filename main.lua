--[[
    Spaghetti Mafia Hub v2.2 (FIXED SIDEBAR & LAYOUT)
    Updates:
    - Fixed Sidebar Buttons disappearing (Separator logic fixed).
    - Preserved "Pyramid" Credits.
    - Preserved Snow & Visuals.
    - Whitelist URL is Raw (Fixed).
]]
...
local Settings = {
    Theme = {
        Gold = Color3.fromRGB(255, 225, 50),
        GoldDim = Color3.fromRGB(180, 150, 20),
...
local function SpawnSnow(parent)
    ...
    flake.TextColor3 = Color3.fromRGB(220, 240, 255)
    flake.Size = UDim2.new(0, math.random(15, 30), 0, math.random(15, 30))
    flake.Position = UDim2.new(math.random(2, 98)/100, 0, -0.1, 0)
    flake.ZIndex = 10
    ...
    local fallInfo = TweenInfo.new(duration, Enum.EasingStyle.Linear)
    TweenService:Create(flake, fallInfo, {Position = UDim2.new(flake.Position.X.Scale, swayAmount, 1.1, 0), Rotation = flake.Rotation + 180}):Play()
    task.delay(duration - 1, function() if flake.Parent then TweenService:Create(flake, TweenInfo.new(1), {TextTransparency = 1}):Play() end end)
...
TweenService:Create(PastaIcon, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {Rotation = 15, Size = UDim2.new(1.1, 0, 0.55, 0)}):Play()
...
SubLoad.BackgroundTransparency = 1; SubLoad.Text = "טוען נתונים..."; 
SubLoad.Font = Enum.Font.Gotham; SubLoad.TextColor3 = Color3.fromRGB(200,200,200); SubLoad.TextSize = 14
SubLoad.ZIndex = 15

task.spawn(function()
    while LoadBox.Parent do SpawnSnow(LoadBox); task.wait(0.1) end
end)
task.wait(2.5)
LoadGui:Destroy()
...
Library:AddGlow(MainFrame, Settings.Theme.Gold, 0.1)

MainFrame.Size = UDim2.new(0,0,0,0)
Library:Tween(MainFrame, {Size = UDim2.new(0, 650, 0, 440)}, 0.6, Enum.EasingStyle.Elastic) 
...
MainTitle.Size = UDim2.new(0,350,0,30); MainTitle.Position = UDim2.new(0,25,0,10)
MainTitle.BackgroundTransparency = 1
MainTitle.Text = "SPAGHETTI <font color='#FFD700'>MAFIA</font> HUB v2"
...
MainSub.Size = UDim2.new(0,300,0,20); MainSub.Position = UDim2.new(0,25,0,35)
MainSub.BackgroundTransparency = 1
MainSub.Text = "עולם הכיף (ULTIMATE)" 
...
CloseBtn.Text = "_"
CloseBtn.TextColor3 = Settings.Theme.Gold
CloseBtn.Font=Enum.Font.GothamBold
CloseBtn.TextSize=18
Library:Corner(CloseBtn, 8)
Library:AddGlow(CloseBtn, Settings.Theme.Gold, 0.7)
...
MiniPasta.MouseButton1Click:Connect(function() MiniPasta.Visible = false; MainFrame.Visible = true; Library:Tween(MainFrame, {Size = UDim2.new(0, 650, 0, 440)}, 0.4, Enum.EasingStyle.Back) end)

--// SIDEBAR FIX (תיקון סרגל צד)
...
-- פס ההפרדה (עכשיו הוא בנפרד ולא מפריע לכפתורים)
local Sep = Instance.new("Frame", Sidebar)
Sep.Size = UDim2.new(0, 2, 0.8, 0); Sep.Position = UDim2.new(1, -2, 0.1, 0); Sep.BorderSizePixel = 0
Sep.BackgroundColor3 = Settings.Theme.Gold
Library:Gradient(Sep, Settings.Theme.Dark, Settings.Theme.Gold, 90)

-- קונטיינר לכפתורים (כדי שהם יסתדרו יפה בלי שהפס יפריע)
local SidebarBtns = Instance.new("Frame", Sidebar)
SidebarBtns.Size = UDim2.new(1, 0, 1, 0)
SidebarBtns.BackgroundTransparency = 1

local SideList = Instance.new("UIListLayout", SidebarBtns); SideList.Padding = UDim.new(0,10); SideList.HorizontalAlignment = Enum.HorizontalAlignment.Center; SideList.SortOrder = Enum.SortOrder.LayoutOrder
local SidePad = Instance.new("UIPadding", SidebarBtns); SidePad.PaddingTop = UDim.new(0,15)
...
local function CreateTab(name, heb, order, isWinter)
    local btn = Instance.new("TextButton", SidebarBtns) -- הורה מתוקן
    ...
    ind.Size = UDim2.new(0, 3, 0.7, 0); ind.Position = UDim2.new(0,0,0.15,0); ind.BackgroundColor3 = Settings.Theme.Gold; ind.Visible = false
    Library:Corner(ind, 4)
    ...
    btn.MouseButton1Click:Connect(function()
        for _,v in pairs(SidebarBtns:GetChildren()) do 
            if v:IsA("TextButton") then 
                Library:Tween(v, {BackgroundColor3 = Color3.fromRGB(12,12,12), TextColor3 = Color3.fromRGB(150,150,150)})
        ...
        Library:Tween(btn, {BackgroundColor3 = Color3.fromRGB(25,25,30), TextColor3 = activeColor})
        ind.BackgroundColor3 = activeColor
        ind.Visible = true
        page.Visible = true
    end)
    
    if order == 1 then -- אם אנחנו רוצים שהטאב הראשון יפתח בהתחלה
        -- אבל אנחנו רוצים את "Winter Event" ראשון ברשימה
    end
    
    -- אם זה אירוע חורף, נפתח אותו כברירת מחדל
    if isWinter then 
        currentTab = btn
        Library:Tween(btn, {BackgroundColor3 = Color3.fromRGB(25,25,30), TextColor3 = Settings.Theme.IceBlue})
        ind.BackgroundColor3 = Settings.Theme.IceBlue
        ind.Visible = true
        page.Visible = true 
    end
            ...
            while Settings.Farming do
                ...
                if char and hrp and target then
                    ...
                    repeat task.wait() 
                        ...
                        if currentDist < 8 then target.CanTouch = true; hrp.CFrame = target.CFrame; if (tick() - stuckStart) > 0.6 then tween:Cancel(); FarmBlacklist[target] = true; break end else stuckStart = tick() end
...
EventSnowContainer.Size = UDim2.new(1,0,1,0); EventSnowContainer.BackgroundTransparency = 1; EventSnowContainer.ClipsDescendants = true; EventSnowContainer.ZIndex = 1
...
Tab_Farm_Scroll.Size = UDim2.new(1, 0, 1, 0); Tab_Farm_Scroll.BackgroundTransparency = 1; Tab_Farm_Scroll.ScrollBarThickness = 2; Tab_Farm_Scroll.ScrollBarImageColor3 = Settings.Theme.IceBlue; Tab_Farm_Scroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; Tab_Farm_Scroll.BorderSizePixel = 0; Tab_Farm_Scroll.ZIndex = 5
...
FarmBtn.Size = UDim2.new(0.95, 0, 0, 70); FarmBtn.BackgroundColor3 = Color3.fromRGB(30, 50, 70); FarmBtn.Text = ""; FarmBtn.LayoutOrder = 1
Library:Corner(FarmBtn, 12); Library:AddGlow(FarmBtn, Settings.Theme.IceBlue)

local FarmTitle = Instance.new("TextLabel", FarmBtn)
FarmTitle.Size = UDim2.new(1, -60, 1, 0); FarmTitle.Position = UDim2.new(0, 20, 0, 0); FarmTitle.Text = "Toggle Auto Farm ❄️\n<font size='13' color='#87CEFA'>הפעלת חווה אוטומטית</font>"; FarmTitle.RichText = true; FarmTitle.TextColor3 = Color3.new(1,1,1); FarmTitle.Font = Enum.Font.GothamBlack; FarmTitle.TextSize = 18; FarmTitle.TextXAlignment = Enum.TextXAlignment.Left; FarmTitle.BackgroundTransparency = 1; FarmTitle.ZIndex = 6
...
task.spawn(function() task.wait(1); if not isFarming then isFarming = true; ToggleFarm(true); if FarmSwitch and FarmDot then Library:Tween(FarmSwitch,{BackgroundColor3=Settings.Theme.IceBlue}); Library:Tween(FarmDot,{Position=UDim2.new(1,-24,0.5,-11)}) end end end)
...
AFKStatus.Size = UDim2.new(0.95, 0, 0, 20); AFKStatus.BackgroundTransparency = 1; AFKStatus.Text = "Anti-AFK: <font color='#00FF00'>Active (Jumper)</font> ⚡"; AFKStatus.RichText = true; AFKStatus.TextColor3 = Color3.new(1, 1, 1); AFKStatus.Font = Enum.Font.GothamMedium; AFKStatus.TextSize = 12; AFKStatus.LayoutOrder = 6; AFKStatus.ZIndex = 6
...
task.spawn(function()
    ...
    while true do
        ...
        pcall(function()
            ...
            ValRed.Text = "+"..tostring(SesC); ValBlue.Text = "+"..tostring(SesS); T_ValR.Text = tostring(CurC); T_ValB.Text = tostring(CurS)
        end)
    end
end)

--// 8. רכיבים וטאבים אחרים (Settings & Main)
local function CreateSlider(parent, title, heb, min, max, default, callback, toggleCallback, toggleName)
    local f = Instance.new("Frame", parent); f.Size = UDim2.new(0.95,0,0,75); f.BackgroundColor3 = Settings.Theme.Box; Library:Corner(f, 8); Library:AddGlow(f, Color3.fromRGB(50,50,50), 0.5)
    ...
    local line = Instance.new("Frame", f); line.Size = UDim2.new(0.9,0,0,12); line.Position = UDim2.new(0.05,0,0.65,0); line.BackgroundColor3 = Color3.fromRGB(30,30,30); Library:Corner(line,6)
    ...
    btn.MouseButton1Down:Connect(function() 
        local move = UIS.InputChanged:Connect(function(i) 
            if i.UserInputType == Enum.UserInputType.MouseMovement then 
                ...
                fill.Size = UDim2.new(r,0,1,0); local v = math.floor(min+((max-min)*r)); l.Text = title.." ("..heb..") : "..v; callback(v) 
            end 
        end)
...
local function CreateSquareBind(parent, id, title, heb, default, callback)
    local f = Instance.new("TextButton", parent); local sizeY = id==3 and 60 or 80; f.Position = id==1 and UDim2.new(0,0,0,0) or (id==2 and UDim2.new(0.52,0,0,0) or UDim2.new(0,0,0,0)); f.Size = UDim2.new(id==3 and 1 or 0.48,0,0,sizeY); f.BackgroundColor3 = Settings.Theme.Box; f.Text=""; f.AutoButtonColor=false; Library:Corner(f, 8); Library:AddGlow(f, Color3.fromRGB(50,50,50), 0.5)
...
CreateSlider(Tab_Main_Page, "Walk Speed", "מהירות הליכה", 1, 250, 16, function(v) Settings.Speed.Value = v end, function(t) Settings.Speed.Enabled = t; if not t and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.WalkSpeed = 16 end end, "Speed")
...
--// 9. טאב קרדיטים (עיצוב פירמידה משודרג)
local CreditBG = Instance.new("Frame", Tab_Credits_Page); CreditBG.Size = UDim2.new(1,0,1,0); CreditBG.BackgroundColor3 = Color3.fromRGB(10,10,12); CreditBG.ZIndex=0; Library:Corner(CreditBG, 0)
...
local function CreateCreditCard(parent, name, role, discord, decal, pos, size)
    local c = Instance.new("Frame", parent); c.Size = size or UDim2.new(0.48, 0, 0, 140); c.Position = pos or UDim2.new(0,0,0,0); c.BackgroundColor3 = Settings.Theme.Box; c.ZIndex = 2; Library:Corner(c, 12); Library:AddGlow(c, Settings.Theme.Gold, 0.5)
    local imgCont = Instance.new("Frame", c); imgCont.Size = UDim2.new(0, 70, 0, 70); imgCont.Position = UDim2.new(0.5, -35, 0.1, 0); imgCont.BackgroundColor3 = Color3.fromRGB(30,30,35); Library:Corner(imgCont, 35)
    local img = Instance.new("ImageLabel", imgCont); img.Size = UDim2.new(1,0,1,0); img.BackgroundTransparency=1; img.Image = "rbxassetid://"..decal; Library:Corner(img,35)
    local tName = Instance.new("TextLabel", c); tName.Size = UDim2.new(1,0,0,20); tName.Position = UDim2.new(0,0,0.6,0); tName.BackgroundTransparency=1; tName.Text = name; tName.Font=Enum.Font.GothamBlack; tName.TextSize=16; tName.TextColor3=Settings.Theme.Gold
    local tRole = Instance.new("TextLabel", c); tRole.Size = UDim2.new(1,0,0,15); tRole.Position = UDim2.new(0,0,0.72,0); tRole.BackgroundTransparency=1; tRole.Text = role; tRole.Font=Enum.Font.GothamBold; tRole.TextSize=12; tRole.TextColor3=Settings.Theme.IceBlue
    local btn = Instance.new("TextButton", c); btn.Size = UDim2.new(0.8, 0, 0, 25); btn.Position = UDim2.new(0.1, 0, 0.85, 0); btn.BackgroundColor3 = Color3.fromRGB(88, 101, 242); btn.Text = "Copy Discord 👾"; btn.TextColor3=Color3.new(1,1,1); btn.Font=Enum.Font.GothamBold; btn.TextSize=12; Library:Corner(btn, 6)
    btn.MouseButton1Click:Connect(function() setclipboard(discord); btn.Text="Copied!"; task.wait(1); btn.Text="Copy Discord 👾" end)
    return c
end

local TopRow = Instance.new("Frame", Tab_Credits_Page); TopRow.Size = UDim2.new(1, 0, 0.45, 0); TopRow.Position = UDim2.new(0,0,0,10); TopRow.BackgroundTransparency=1; TopRow.ZIndex=5
CreateCreditCard(TopRow, "Neho", "Founder", "nx3ho", "117903609576144", UDim2.new(0.02, 0, 0, 0))
CreateCreditCard(TopRow, "BadShot", "CoFounder", "8adshot3", "94534356128704", UDim2.new(0.51, 0, 0, 0))
local BotRow = Instance.new("Frame", Tab_Credits_Page); BotRow.Size = UDim2.new(1, 0, 0.45, 0); BotRow.Position = UDim2.new(0,0,0.42,0); BotRow.BackgroundTransparency=1; BotRow.ZIndex=5
CreateCreditCard(BotRow, "xyth", "Community Manager", "sc4rlxrd", "0", UDim2.new(0.26, 0, 0, 0))
...
print("[SYSTEM] Spaghetti Mafia Hub v2.2 (Ultimate Fix) Loaded")
