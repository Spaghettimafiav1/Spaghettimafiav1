--[[
    Spaghetti Mafia Hub v2.5 (PREMIUM EDITION)
    
    Update Log:
    1. VISUALS: Deep Black Theme (10,10,10) + Neon Gold Glow.
    2. ANIMATION: Floating Loading Screen, Slide Tabs, Scale Hover.
    3. LOGIC: R15/R6 Bang updated to CFrame Math (No Animations).
    4. PRESERVATION: ALL Original Logic (Farm, Scan, Fly, Storm) kept intact.
]]
...
local Settings = {
    Theme = {
        Gold = Color3.fromRGB(255, 200, 0), -- Updated to Premium Gold
        Dark = Color3.fromRGB(10, 10, 10), -- Updated to Deep Matte Black
        Box = Color3.fromRGB(18, 18, 18), -- Lighter Black for elements
...
--// SOUND SYSTEM
local Sounds = {
    Click = "rbxassetid://6895079853", -- Updated to Mechanical Click
    Hover = "rbxassetid://6895079980", -- New Hover Sound
...
--// 3. UI FUNCTIONS (ENHANCED FOR PREMIUM FEEL)
local Library = {}
...
--// HELPER FOR SITTING ANIMATION (Updated Logic)
local function PlaySit(play)
    if play then
        ...
        if hum then
            hum.Sit = true
            -- We just set sit to true, the loop handles position
        end
    else
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChild("Humanoid")
...
LoadBox.ClipsDescendants = false
...
-- Drop Shadow
local Shadow = Instance.new("ImageLabel", LoadBox)
Shadow.Size = UDim2.new(1, 40, 1, 40); Shadow.Position = UDim2.new(0,-20,0,-20)
Shadow.BackgroundTransparency=1; Shadow.Image="rbxassetid://6015897843"; Shadow.ImageColor3=Color3.new(0,0,0); Shadow.ImageTransparency=0.4; Shadow.ZIndex=0
...
PastaIcon.BackgroundTransparency = 1; PastaIcon.Text = "🍝"; PastaIcon.TextSize = 70; PastaIcon.ZIndex = 15
...
task.spawn(function()
    ...
    while LoadBox.Parent do
        t = t + 0.1
        PastaIcon.Position = UDim2.new(0, 0, 0.05 + math.sin(t)*0.05, 0)
        PastaIcon.Rotation = math.sin(t/2) * 8
        task.wait(0.03)
    end
end)
...
TitleLoad.BackgroundTransparency = 1; TitleLoad.Text = "Spaghetti Mafia v2"; 
...
-- Glow on Bar
local BarGlow = Instance.new("UIStroke", LoadingBarFill); BarGlow.Color = Settings.Theme.Gold; BarGlow.Thickness = 1; BarGlow.Transparency = 0.2

Library:Tween(LoadingBarFill, {Size = UDim2.new(1, 0, 1, 0)}, 2.5, Enum.EasingStyle.Exponential)
...
Library:AddGlow(MainFrame, Settings.Theme.Gold, 2)

-- Main Shadow for Float Effect
local MainShadow = Instance.new("ImageLabel", MainFrame)
MainShadow.Size = UDim2.new(1, 50, 1, 50); MainShadow.Position = UDim2.new(0,-25,0,-25)
MainShadow.BackgroundTransparency=1; MainShadow.Image="rbxassetid://6015897843"; MainShadow.ImageColor3=Color3.new(0,0,0); MainShadow.ImageTransparency=0.3; MainShadow.ZIndex=-1

MainFrame.Size = UDim2.new(0,0,0,0); Library:Tween(MainFrame, {Size = UDim2.new(0, NEW_WIDTH, 0, NEW_HEIGHT)}, 0.6, Enum.EasingStyle.Back) 
...
local MainTitle = Instance.new("TextLabel", TopBar); MainTitle.Size = UDim2.new(0,300,0,30); MainTitle.Position = UDim2.new(0,25,0,10); MainTitle.BackgroundTransparency = 1; MainTitle.Text = "SPAGHETTI <font color='#FFC800'>MAFIA</font> v2"; MainTitle.RichText = true; MainTitle.Font = Enum.Font.GothamBlack; MainTitle.TextSize = 24; MainTitle.TextColor3 = Color3.new(1,1,1); MainTitle.TextXAlignment = Enum.TextXAlignment.Left
...
MainSub.Text = "עולם הכיף - Premium" 
...
local CloseBtn = Instance.new("TextButton", TopBar); CloseBtn.Size = UDim2.new(0, 30, 0, 30); CloseBtn.Position = UDim2.new(1, -45, 0, 15); CloseBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30); CloseBtn.Text = "×"; CloseBtn.TextColor3 = Settings.Theme.Gold; CloseBtn.Font=Enum.Font.GothamBold; CloseBtn.TextSize=22; Library:Corner(CloseBtn, 8); Library:AddGlow(CloseBtn, Settings.Theme.Gold, 1.5)
...
task.spawn(function()
    ...
    if StormValue then
        ...
        TimerWidget.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
        TimerWidget.BorderSizePixel = 0
        Library:Corner(TimerWidget, 10)
        Library:AddGlow(TimerWidget, Settings.Theme.Gold, 0.5)
        ...
        local function UpdateStormTimer(val)
            ...
            if val <= 0 then
                ...
                if val <= 30 then
                    ...
                    T_Time.TextSize = 18
                    T_Time.TextColor3 = Color3.fromRGB(255, 255, 255)
                    TweenService:Create(TimerWidget, TweenInfo.new(0.5), {BackgroundColor3 = Color3.fromRGB(15, 15, 15)}):Play()
                end
            end
        end
...
Library:AddGlow(UserProfile, Settings.Theme.Gold, 0.5)

local AvatarFrame = Instance.new("Frame", UserProfile)
AvatarFrame.Size = UDim2.new(0, 50, 0, 50)
AvatarFrame.Position = UDim2.new(0.5, -25, 0.1, 0)
AvatarFrame.BackgroundColor3 = Settings.Theme.Gold
AvatarFrame.BorderSizePixel = 0
AvatarFrame.ZIndex = 11
local AvatarCorner = Instance.new("UICorner", AvatarFrame); AvatarCorner.CornerRadius = UDim.new(1, 0) -- Circle
...
UsernameText.Size = UDim2.new(1, 0, 0, 20)
UsernameText.Position = UDim2.new(0, 0, 0.75, 0)
...
SideBtnContainer.Size = UDim2.new(1, 0, 1, -90) 
SideBtnContainer.BackgroundTransparency = 1
SideBtnContainer.ScrollBarThickness = 0
...
Container.ClipsDescendants = true -- Needed for slide animation
...
local function CreateTab(name, heb, order, isWinter)
    ...
    btn.Text = "   " .. name .. "\n   <font size='10' color='#777'>"..heb.."</font>"
    btn.RichText = true
    ...
    local PagePad = Instance.new("UIPadding", page); PagePad.PaddingTop=UDim.new(0,15); PagePad.PaddingLeft=UDim.new(0,15); PagePad.PaddingRight=UDim.new(0,15); PagePad.PaddingBottom=UDim.new(0,15)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        local activeBG = isWinter and Settings.Theme.IceDark or Color3.fromRGB(25, 25, 30)
...
Library:AddGlow(FarmBtn, Settings.Theme.IceBlue, 1.5)
Library:AddHover(FarmBtn)
...
FarmBtn.MouseButton1Click:Connect(function() 
    ...
    if isFarming then 
        ...
        Library:Tween(FarmDot,{Position=UDim2.new(0,2,0.5,-12)}) 
    end 
end)

-- Balance Stats (PRESERVED)
...
local TotBlues = Instance.new("Frame", BalanceContainer); TotBlues.BackgroundColor3 = Color3.fromRGB(15, 30, 50); Library:Corner(TotBlues, 12); local StrokeTotalB = Library:AddGlow(TotBlues, Settings.Theme.ShardBlue, 1.5)
...
local TotReds = Instance.new("Frame", BalanceContainer); TotReds.BackgroundColor3 = Color3.fromRGB(30, 15, 15); Library:Corner(TotReds, 12); local StrokeTotalR = Library:AddGlow(TotReds, Settings.Theme.CrystalRed, 1.5)
...
local BoxBlue = Instance.new("Frame", StatsContainer); BoxBlue.BackgroundColor3 = Color3.fromRGB(15, 30, 50); Library:Corner(BoxBlue, 12); local StrokeBlue = Library:AddGlow(BoxBlue, Settings.Theme.IceBlue, 1.5)
...
local BoxRed = Instance.new("Frame", StatsContainer); BoxRed.BackgroundColor3 = Color3.fromRGB(30, 15, 15); Library:Corner(BoxRed, 12); local StrokeRed = Library:AddGlow(BoxRed, Settings.Theme.CrystalRed, 1.5)
...
--// 8. COMPONENTS (SLIDERS & BINDS) - PREMIUM UI
local function CreateSlider(parent, title, heb, min, max, default, callback, toggleCallback, toggleName)
    ...
    Library:Corner(f, 12)
    Library:AddGlow(f, Settings.Theme.Gold, 1.2)
    Library:AddHover(f)
...
--// 10. TARGET TAB (PLAYERS & TOOLS) - REDESIGNED LAYOUT
...
TargetScroll.ScrollBarThickness = 0 
TargetScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
TargetScroll.CanvasSize = UDim2.new(0,0,0,0)

-- SPLIT LAYOUT (LEFT/RIGHT)
local TargetSplit = Instance.new("Frame", TargetScroll); TargetSplit.Size=UDim2.new(1,0,0,160); TargetSplit.BackgroundTransparency=1
local LeftSide = Instance.new("Frame", TargetSplit); LeftSide.Size=UDim2.new(0.3,0,1,0); LeftSide.BackgroundTransparency=1
local RightSide = Instance.new("Frame", TargetSplit); RightSide.Size=UDim2.new(0.68,0,1,0); RightSide.Position=UDim2.new(0.32,0,0,0); RightSide.BackgroundTransparency=1

-- AVATAR BOX (LEFT)
local AvatarBox = Instance.new("Frame", LeftSide)
AvatarBox.Size = UDim2.new(1,0,1,0)
AvatarBox.BackgroundColor3 = Settings.Theme.Box
Library:Corner(AvatarBox, 12)
Library:AddGlow(AvatarBox, Settings.Theme.Gold, 1.5)

local TargetAvatar = Instance.new("ImageLabel", AvatarBox)
TargetAvatar.Size = UDim2.new(0.8,0,0.6,0); TargetAvatar.Position=UDim2.new(0.1,0,0.1,0); TargetAvatar.BackgroundTransparency=1; TargetAvatar.Image="rbxassetid://0"
Library:Corner(TargetAvatar, 100) -- Circle

local TargetStatus = Instance.new("TextLabel", AvatarBox)
TargetStatus.Size=UDim2.new(1,0,0,20); TargetStatus.Position=UDim2.new(0,0,0.75,0); TargetStatus.BackgroundTransparency=1; TargetStatus.Text="WAITING"; TargetStatus.TextColor3=Color3.fromRGB(150,150,150); TargetStatus.Font=Enum.Font.GothamBold

-- INPUTS (RIGHT)
local TargetInput = Instance.new("TextBox", RightSide)
TargetInput.Size=UDim2.new(1,0,0,45); TargetInput.BackgroundColor3=Settings.Theme.Box; TargetInput.Text=""; TargetInput.PlaceholderText="Player Name..."; TargetInput.TextColor3=Color3.new(1,1,1); TargetInput.Font=Enum.Font.GothamBold; TargetInput.TextSize=16
Library:Corner(TargetInput, 8); Library:AddGlow(TargetInput, Settings.Theme.Gold, 1.5)

-- ACTIONS GRID
local ActionGrid = Instance.new("ScrollingFrame", RightSide)
ActionGrid.Size=UDim2.new(1,0,0,100); ActionGrid.Position=UDim2.new(0,0,0.35,0); ActionGrid.BackgroundTransparency=1; ActionGrid.BorderSizePixel=0
local ActionLayout = Instance.new("UIGridLayout", ActionGrid); ActionLayout.CellSize=UDim2.new(0.48,0,0,40); ActionLayout.CellPadding=UDim2.new(0.04,0,0.1,0)
...
TargetInput.FocusLost:Connect(function()
    ...
    if p then
        ...
        PlaySound(Sounds.Click)
        TargetAvatar.Image = Players:GetUserThumbnailAsync(p.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size150x150)
        TargetStatus.Text = "ONLINE"; TargetStatus.TextColor3 = Color3.fromRGB(50,255,100)
    else
        TargetAvatar.Image = "rbxassetid://0"
        TargetStatus.Text = "OFFLINE"; TargetStatus.TextColor3 = Color3.fromRGB(255,50,50)
    end
end)
...
-- R15 CFRAME MATH BANG (REPLACED AS REQUESTED)
local TrollConnection = nil
CreateToggleBtn(ActionGrid, "BANG (CFrame)", function(state)
    if not state then
    ...
    if target and target.Character and LocalPlayer.Character then
        ...
        -- Procedural Math Loop (Works on R15 & R6)
...
CreateToggleBtn(ActionGrid, "SPECTATE", function(state)
    local target = GetPlayer(TargetInput.Text)
...
CreateToggleBtn(ActionGrid, "HEADSIT", function(state)
...
CreateToggleBtn(ActionGrid, "BACKPACK", function(state)
    isSittingAction = state 
    if not state then
        PlaySit(false) 
...
ScannerBox.Size = UDim2.new(1, 0, 0, 250)
ScannerBox.Position = UDim2.new(0,0,0,170)
...
Library:AddGlow(RejoinBtn, Color3.fromRGB(200, 60, 60), 1.5)
...
local function CreateCreditCard(parent, name, role, discord, decal, pos, size)
    ...
    c.ZIndex = 2
    Library:Corner(c, 12)
    Library:AddGlow(c, Settings.Theme.Gold, 1.5)
    Library:AddHover(c)
...
for _, btn in pairs(SideBtnContainer:GetChildren()) do
    if btn:IsA("TextButton") then
        btn.MouseEnter:Connect(function() Library:Tween(btn, {Size = UDim2.new(0.95, 0, 0, 45)}, 0.3, Enum.EasingStyle.Quart) end)
        btn.MouseLeave:Connect(function() Library:Tween(btn, {Size = UDim2.new(0.9, 0, 0, 45)}, 0.3, Enum.EasingStyle.Quart) end)
    end
end
...
print("[SYSTEM] Spaghetti Mafia Hub v2.5 (Premium Redesign) Loaded")
